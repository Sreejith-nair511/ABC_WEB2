...
// Add mobile-optimized default options
const getMobileOptimizedOptions = (baseOptions: HyperspeedOptions): HyperspeedOptions => {
  ...
  return {
    ...
    totalSideLightSticks: 8, // Reduced from 20
    lightPairsPerRoadWay: 15, // Reduced from 40
    carLightsFade: 0.6, // Increased fade for better performance
    // Reduce geometry complexity
    length: 200, // Reduced from 400
    // Reduce post-processing intensity
    fov: 85, // Slightly reduced
...
class Road {
  ...
  createPlane(side: number, width: number, isRoad: boolean) {
    const options = this.options;
    const segments = isMobileDevice() ? 50 : 100; // Reduce segments on mobile
    const geometry = new THREE.PlaneGeometry(
      isRoad ? options.roadWidth : options.islandWidth,
      options.length,
      isMobileDevice() ? 10 : 20, // Reduce width segments on mobile
      segments
    );
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    this.options = getMobileOptimizedOptions(options);
    ...
    // Conservative pixel ratio for mobile
  ...
  initPasses() {
    ...
    this.bloomPass = new EffectPass(
      this.camera,
      new BloomEffect({
        luminanceThreshold: isMobile ? 0.4 : 0.2, // Higher threshold on mobile
        luminanceSmoothing: 0,
        resolutionScale: isMobile ? 0.5 : 1, // Lower resolution on mobile
      })
    );
    ...
    const smaaPass = new EffectPass(
      this.camera,
      new SMAAEffect({
        preset: isMobile ? SMAAPreset.LOW : SMAAPreset.MEDIUM, // Lower quality on mobile
      })
    );
  ...
  tick() {
    ...
    if (delta >= frameTime || !isMobileDevice()) {
      this.render(delta);
      this.update(delta);
    }
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  const mergedOptions: HyperspeedOptions = getMobileOptimizedOptions({
    ...defaultOptions,
    ...effectOptions,
  });
